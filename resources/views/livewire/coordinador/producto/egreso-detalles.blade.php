        ...
        <div class="card author-box">
            <div class="card-body">
                <div class="author-box-center">
                    <img alt="image" src="{{ Avatar::create($egreso->codigo)->setFontSize(35)->setChars(3) }}"
                        ...
                        <table class="table table-striped">
                            ...
                            <tbody>
                                @foreach ($egreso->productos as $producto)
                                    <tr>
                                        <td><a @isset($producto->deleted_at)
                                                    style="text-decoration: line-through"
                                                @endisset
                                                class="btn-link text-wrap">{{ $producto->nombre }}</a></td>
                                        <td>{{ $producto->pivot->cantidad_real }} ({{ $producto->medida->simbolo }})
                                        </td>
                                        <td class="text-center">
                                            {{ number_format($producto->pivot->total, 3, ',', '.') }}</td>
                                    </tr>
                                @endforeach
                            </tbody>
                        </table>
