...
@section('breadcrumb')
    ...
    <li class="breadcrumb-item"><a href="{{ route('coordinador.requerimiento.show', $id) }}"><i
                class="fas fa-clipboard-list"></i> Requerimiento # {{ $id }}</a></li>
    <li class="breadcrumb-item active" aria-current="page"> Atención </li>
@endsection
@section('css')
            ...
            <div class="card-body">
                ...
                <div class="form-row">
                    ...
                    <div class="form-group col-sm-12 col-lg-4">
                        ...
                        <model-list-select :list="operadores" v-model="operador_id" :class="hasOperador"
                            option-value="id" option-text="nombres" placeholder="Elije Un Operador"
                            {{-- @input="operadorEmit" --}}>
                            <p class="error-message text-danger font-weight-bold" v-if="errors.operador_id">
                                @{{ errors . operador_id[0] }}</p>

                    </div>
                    @endrole
                    <div class="form-group col">
                        <label for="inputAddress">Fecha de Atencion</label>
                        <input type="date" v-model="fecha_atencion"
                            class="form-control @error('fecha_atencion') is-invalid @enderror" placeholder="">
                        <p class="error-message text-danger font-weight-bold" v-if="errors.fecha_atencion">
                            @{{ errors . fecha_atencion[0] }}</p>
                    ...
                    <div class="form-group col-sm-12 col-md-12">
                        ...
                        <p class="error-message text-danger font-weight-bold" v-if="errors.detalle_atencion">
                            @{{ errors . detalle_atencion[0] }}</p>
                        <textarea cols="30" rows="10" v-model="detalle_atencion"
                    ...
                    <div class="form-group col-sm-12 col-md-12">
                        ...
                        <p class="error-message text-danger font-weight-bold" v-if="errors.observacion">
                            @{{ errors . observacion[0] }}</p>
                    ...
                    <div class="form-group col-sm-12 col-md-12">
                        ...
                        <gmap-map :center="center" :zoom="12" style="width:100%;  height: 350px;"
                            ...
                            <gmap-marker v-if="puntuacion.lat !== ''" :position="puntuacion"
                                icon="https://maps.google.com/mapfiles/kml/paddle/grn-circle.png"></gmap-marker>
                        </gmap-map>
                        <p class="error-message text-danger font-weight-bold" v-if="errors.latitud">
                            @{{ errors . latitud[0] }}</p>
                        <p class="error-message text-danger font-weight-bold" v-if="errors.longitud">
                            @{{ errors . longitud[0] }}</p>

                    </div>

                    <div class="form-group col-sm-12 col-lg-6">
                        <h2 for="inputAddress">AGREGAR EGRESO <input type="checkbox" v-model="has_egreso"
                                class="form-check-inline"></h2>
...
<div class="col-lg-8 col-sm-12" v-if="has_egreso">
    <div class="card">
        <div class="p-2">
            <div class="card-head">
                <h2 class="text-center text-danger font-weight-bold">EGRESO</h2>
            </div>
            <div class="card-body">
                ...
                <div class="form-row">
                    <div class="form-group col-lg-6 col-sm-12">
                        ...
                        <p class="error-message text-danger font-weight-bold" v-if="errors.codigo">
                            @{{ errors . codigo[0] }}</p>
                    ...
                    <div class="form-group col-lg-6 col-sm-12">
                        ...
                        <p class="error-message text-danger font-weight-bold" v-if="errors.total_egreso">
                            @{{ errors . total_egreso[0] }}</p>
                    ...
                    <div class="form-group col-lg-12 col-sm-12">
                        ...
                        <textarea name="" id="" cols="30" rows="10" class="form-control"
                            v-model="egreso.descripcion"></textarea>
                        <p class="error-message text-danger font-weight-bold" v-if="errors.descripcion">
                            @{{ errors . descripcion[0] }}</p>
                ...
                <div class="border p-2">
                    ...
                    <div class="form-row justify-content-center">
                        <div class="form-group col-lg-4 col-sm-12">
                            <select name="" v-model="egreso.producto_id" id="" class="form-control"
                                @change="changeUnidad">
                        ...
                        <div class="form-group col-lg-2 col-sm-12">
                            <input type="number" class="form-control text-right" v-model="egreso.cantidad"
                                min="1" @change="conversion" placeholder="cantidad">
                        </div>
                        <div class="form-group col-lg-3 col-sm-12">
                            <select name="" id="" v-model="egreso.unidad_id" class="custom-select"
                                @change="conversion">
                                <option value="" selected="" disabled="">Seleccione Unidad Base</option>
                                <option v-for="(unidad, index) in medidas" :value="unidad.id">
                                    @{{ unidad . unidad }}, @{{ unidad . simbolo }}</option>
                            </select>
                        </div>
                        <div class="form-group col-lg-2 col-sm-12">
                            <input type="number" class="form-control text-right" disabled=""
                                v-model="egreso.cantidad_real" min="1" placeholder="Cant">
                        </div>
                        <div class="form-group col-lg-2 col-sm-12">
...
<button class="btn btn-success" :disabled="buttonDisable" @click.prevent="submitAtencion()">Enviar
    Atencion</button>
</div>
{{-- @livewire('component.atencion-requerimiento') --}}
</div>
@endsection
...
<script type="text/javascript">
    ...
    //let coordinador = (@json(Auth::user()->hasRole('coordinador')));
    //	 let operador = (@json(Auth::user()->hasRole('operador')));
    let id = @json($id);
    ...
    const atencion = new Vue({
        ...
        methods: {
            ...
            submitAtencion() {
                if (this.egreso.items.length <= 0 && this.has_egreso == true) {
                    return iziToast.error({
                        title: 'Operaciones',
                        message: 'No has agregado un producto a la lista',
                        position: 'topRight'
                    });
                }
            ...
            calcularfactor() {
                let find = this.conversiones.filter(x => x.medida_base == this.egreso.medida.id && x
                    .medida_conversion == this.egreso.unidad_id);
                if (find.length == 1) {
            ...
            agregarItem() {
                if (this.egreso.producto_id === '') {
                    ...
                    let item = {
                        id: this.egreso.producto_id,
                        nombre: producto[0].nombre,
                        medida_base: this.egreso.unidad_id,
                        medida_conversion: this.egreso.medida.id,
                        cantidad_unidad: Number(this.egreso.cantidad),
                        cantidad_base: this.egreso.cantidad_real,
