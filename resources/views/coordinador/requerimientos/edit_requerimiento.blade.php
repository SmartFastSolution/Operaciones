...
@section('breadcrumb')
    ...
    <li class="breadcrumb-item active" aria-current="page"><i class="fas fa-edit"></i> Requerimiento #
        {{ $requerimiento->id }}</li>
@endsection
@section('css')
...
<h1 class="text-danger text-center">Editar Requerimiento</h1>
...
<div class="col-lg-6">
    <div class="card" id="requerimiento">
        <div class="card-body">
            <h3 class="font-weight-bold text-danger">Archivos del requerimiento</h3>
            ...
            <div class="table-responsive" style="height: 320px; overflow-y: scroll">
                <table class="table table-striped">
                    ...
                    <tbody>
                        ...
                        <tr v-for="(documento, index) in req.documentos">
                            ...
                            <td width="25"><a href="" @click.prevent="deleteDoc(index)"
...
<div class="col-lg-6">
    <div class="card" id="requerimiento">
        <div class="card-body">
            <h3 class="font-weight-bold text-danger">Archivos a eliminar</h3>
            <div class="table-responsive" style="height: 320px; overflow-y: scroll">
                <table class="table table-striped">
                    <thead>
                        <tr>
                    ...
                    <tbody>
                        {{-- @foreach ($requerimiento->documentos as $documento) --}}
                        <tr v-for="(documento, index) in  eliminados">
                            <td>@{{ documento . nombre }}</td>
                            <td>@{{ documento . extension }}</td>
                            <td width="25"><a target="_blank" :href="documento . archivo"
                                    class="btn btn-primary"><i class="fa fa-eye"></i></a></td>
                            <td width="25"><a href="" @click.prevent="reintegrar(index)"
                                    class="btn btn-success"><i class="fa fa-arrow-circle-left"></i></a>
                            </td>
                        </tr>
                        {{-- @endforeach --}}
                    </tbody>
...
<script type="text/javascript">
    ...
    const requerimiento = new Vue({
        ...
        mounted() {
            this.editRequerimiento();
        },

        methods: {
            deleteDoc(index) {
                // console.log(_.pluck(this.req.documentos, 'id'));
                documento = this.req.documentos[index];
                this.eliminados.push(documento);
                this.req.documentos.splice(index, 1);

            },
            reintegrar(index) {
                documento = this.eliminados[index];
                this.req.documentos.push(documento);
                this.eliminados.splice(index, 1);
            },
            editRequerimiento() {
                this.observacion_requerimiento = req.observacion
                this.requerimiento = req.requerimiento
                this.cuenta_requerimiento = req.cuenta
                this.nombre_requerimiento = req.nombres
                this.cedula_requerimiento = req.cedula
                this.codigo_requerimiento = req.codigo
                this.telefonos_requerimiento = req.telefonos
                this.correos_requerimiento = req.correos
                this.direccion_requerimiento = req.direccion
                this.fecha_requerimiento = req.fecha_maxima
                this.detalle_requerimiento = req.detalle
                this.codigo_catastral = req.codigo_catastral
                this.sector = req.sector_id
                this.requerimiento = req.tipo_requerimiento_id
                this.center = {
                    lat: req.latitud,
                    lng: req.longitud,
            ...
            submitRequerimiento() {
                ...
                data.append('eliminados', JSON.stringify(_.pluck(this.eliminados, 'id')));
                for (var i = 0; i < this.archivos.length; i++) {
                    data.append('archivos[]', this.archivos[i].file);
                }
                axios.post('/coordinador/requerimiento/' + req.id + '/update', data, config)
                    .then(function(res) {
